...
- If Linux release packaging collects Fastforge outputs, then search `dist` recursively (Fastforge writes into `dist/<version>/...`) before renaming/uploading artifacts.
...
- If a native Android plugin can throw `Error` during registration, then use a custom `MainActivity.configureFlutterEngine` registrant with per-plugin `Throwable` guards so one plugin failure does not block all channels.
